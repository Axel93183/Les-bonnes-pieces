...
function genererPieces(pieces) {
  for (let i = 0; i < pieces.length; i++) {
    ...
    pieceElement.dataset.id = pieces[i].id // Affecte un attribut personnalisé "data-id" à l'élément article.
...
const boutonFiltreDescription = document.querySelector(
...
inputRange.addEventListener("input", () => {
  ...
  const piecesFiltrees = pieces.filter(
    (piece) => piece.prix <= inputRange.value
  );
  //console.log(piecesFiltrees);
